...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans text-gray-900 antialiased">
            ...
            <ul class="space-y-2 font-medium">
                <li class="bg-gray-900 rounded-lg">
                    <a href="{{ route('empleado.index') }}" class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
                        ...
                        <span class="ms-3">{{ Auth::guard('empleado')->user()->usuario }}</span>
                    </a>
                </li>
                <li>
                    <a href="{{ route('citas.filtros') }}" class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
                ...
                <li>
                    <a href="{{ route('citas.prioridad') }}" class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
                ...
                <li>
                    <a href="{{ route('autos.filtros') }}" class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
                ...
                <li>
                    <form action="{{ route('empleado.logout') }}" method="POST">
                        @csrf <!-- Asegúrate de incluir el token CSRF para la seguridad de la solicitud -->
                        <button type="submit" class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
                            <svg class="flex-shrink-0 w-5 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
        ...
        <div class="p-4 sm:ml-64 md:ml-64 pt-2">
            <div>
                {{ $slot }}
            </div>
        </div>
